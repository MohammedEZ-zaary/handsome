...
httpServer::parseHTTPRequest(const std::string &httpRequest) {
...
void httpServer::acceptConnections() {
  while (true) {
    ...
    // MultiThreads
    if (httpServer::MULTI_THREAD) {
      std::thread clinet_request_thread(
...
// Not Good at multiThreading -> Race condition at req/std::cerr bug #06
// read/write req For more saftiy pass const clientSocket
...
// Good at multiThreading
std::map<std::string, std::string>
httpServer::parseHttpHeaderRequest(const std::string &request) {
...
void httpServer::handleRequestHeader(requestHeader &req, const std::string &key,
...
void httpServer::handleRequestBody(requestHeader &req,
                                   const std::string &value) {
...
void httpServer::handleQueryParams(requestHeader &req,
                                   const std::string &value) {
  ...
  if (questionMark != std::string::npos) {
    ...
    while (std::getline(ss, item, '&')) {
      ...
      if (equalsPos != std::string::npos) {
        ...
        req.queryParams[key] =
            httpServer::urlDecoded(value); // Store in the map
      }
    }
  }
}
